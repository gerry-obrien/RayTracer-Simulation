# -*- coding: utf-8 -*-
"""
Created on Fri Dec  3 16:01:54 2021
...
def parax_focus1():
    ...
    of the plano-convex lens with the flat side towards the input
    """
...
print('Paraxial focal point =', parax_focus1())
...
for i in range(len(bun)):
    ...
    output = g.OutputPlane(parax_focus1())
#    output = g.OutputPlane(400)
    output.propagate_ray(ray)
    # print(ray.vertices())
    plt.title('Ray trace through the Plano-Convex lens with the flat side towards the input')
    plt.xlabel('z (mm)')
    plt.ylabel('y (mm)')
    plt.grid()
...
#    output = g.OutputPlane(400)
    output.propagate_ray(ray)
    plt.title('Cross section at the output for the Plano-Convex lens with the flat side towards the input')
    plt.xlabel('x (mm)')
    plt.ylabel('y (mm)')
    # print(ray.vertices())
